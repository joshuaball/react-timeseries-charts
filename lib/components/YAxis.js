...
var YAxis = function (_React$Component) {
    ...
    _createClass(YAxis, [{
        ...
        key: "retrieveStyles",
        value: function retrieveStyles(style) {
            return {
                labelStyle: _extends({}, style.label),
                valueStyle: _extends({}, style.values),
                axisStyle: _extends({}, style.axis),
                tickStyle: _extends({}, style.ticks)
            };
        }
    }, {
        key: "postSelect",
        value: function postSelect(style, hideAxisLine, height) {
            var _retrieveStyles = this.retrieveStyles(style),
                valueStyle = _retrieveStyles.valueStyle,
                tickStyle = _retrieveStyles.tickStyle,
                axisStyle = _retrieveStyles.axisStyle;

            var tickClasses = this.props.baseStyleClassRoot + "yAxis__ticks " + (tickStyle.classes || "");
            var valueClasses = this.props.baseStyleClassRoot + "yAxis__values " + (valueStyle.classes || "");
            var axisClasses = this.props.baseStyleClassRoot + "yAxis__axis " + (axisStyle.classes || "");
            delete tickStyle.classes;
            delete valueStyle.classes;
            delete axisStyle.classes;

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("text").classed(valueClasses, true).styles(valueStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("line").classed(tickClasses, true).styles(tickStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".domain").remove();

            if (!hideAxisLine) {
                (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").append("line").classed(axisClasses, true).styles(axisStyle).attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", height);
            }
        }
    }, {
        key: "generator",
        value: function generator(type, absolute, yformat, axis, scale, height, tickCount, min, max) {
            var axisGenerator = void 0;
            if (type === "linear" || type === "power") {
                if (tickCount > 0) {
                    var stepSize = (max - min) / (tickCount - 1);
                    axisGenerator = axis(scale).tickValues((0, _d3Array.range)(min, max + max / 10000, stepSize)).tickFormat(function (d) {
                        if (absolute) {
        ...
        value: function renderAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
            ...
            var _retrieveStyles2 = this.retrieveStyles(this.props.style),
                labelStyle = _retrieveStyles2.labelStyle,
                valueStyle = _retrieveStyles2.valueStyle;

            var labelClasses = this.props.baseStyleClassRoot + "yAxis__label yaxislabel " + (labelStyle.classes || "");
            var valueClasses = this.props.baseStyleClassRoot + "yAxis__values " + (valueStyle.classes || "");
            ...
            // Add the new axis
            this.axis = (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).append("g").attr("transform", "translate(" + x + ",0)").attr("class", "yaxis").classed(valueClasses, true).styles(valueStyle).call(axisGenerator.tickSize(tickSize)).append("text").text(label || this.props.label).classed(labelClasses, true).styles(labelStyle).attr("transform", "rotate(-90)").attr("y", labelOffset).attr("dy", ".71em").attr("text-anchor", "end");

            this.postSelect(this.props.style, hideAxisLine, height);
        }
    }, {
        key: "updateAxis",
        value: function updateAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
            var yformat = this.yformat(fmt);
            var axis = align === "left" ? _d3Axis.axisLeft : _d3Axis.axisRight;
            var tickSize = showGrid && this.props.isInnerAxis ? -chartExtent : 5;
        ...
        value: function updateLabel(label) {
            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select(".yaxislabel").text(label);
        }
    }, {
        key: "render",
